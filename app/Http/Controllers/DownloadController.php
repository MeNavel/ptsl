...
{
    public function downloadExcelSidomulyo()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        // Ambil data dari database
        $data = Sidomulyo::select('*')->get();
        ...
        $sheet->setCellValue('BS1', 'Nomor Handphone');
        ...
        foreach ($data as $item) {
            ...
            if ($item->Tanggal_Lahir != "") {
                $Tanggal_Lahir = Carbon::createFromFormat('Y-m-d', $item->Tanggal_Lahir)->format('d-m-Y');
            } else {
                $Tanggal_Lahir = null;
            }
            if ($item->An_Tanggal_Lahir != "") {
                $An_Tanggal_Lahir = Carbon::createFromFormat('Y-m-d', $item->An_Tanggal_Lahir)->format('d-m-Y');
            } else {
            ...
            $sheet->setCellValue('D' . $row, $Tgl_Pendataan);
            ...
            $sheet->setCellValue('O' . $row, $Tanggal_Lahir);
            ...
            $sheet->setCellValue('W' . $row, $An_Tanggal_Lahir);
            ...
            $sheet->setCellValue('BS' . $row, $item->No_HP);
            $row++;
        }
        // Simpan file excel
        $writer = new Xlsx($spreadsheet);
        $writer->save('Nominatif Pendaftaran PTSL Desa Sidomulyo.xlsx');

        // Download file excel
        return response()->download(public_path('Nominatif Pendaftaran PTSL Desa Sidomulyo.xlsx'))->deleteFileAfterSend(true);
    }

    public function downloadExcelSidorejo()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        // Ambil data dari database
        $data = Sidorejo::select('*')->get();
        ...
        // Simpan file excel
        $writer = new Xlsx($spreadsheet);
        $writer->save('Nominatif Pendaftaran PTSL Desa Sidorejo.xlsx');

        // Download file excel
        return response()->download(public_path('Nominatif Pendaftaran PTSL Desa Sidorejo.xlsx'))->deleteFileAfterSend(true);
    }
    ...
    public function downloadExcelSumberagung()
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        // Ambil data dari database
        $data = Sumberagung::select('*')->get();
        ...
        // Download file excel
        return response()->download(public_path('Nominatif Pendaftaran PTSL Desa Sumberagung.xlsx'))->deleteFileAfterSend(true);
    }

    public function exportSidomulyo($No_Nominatif)
    {
        $data = Sidomulyo::find($No_Nominatif);
        $phpWord = new \PhpOffice\PhpWord\TemplateProcessor('berkas2024.docx');
    ...
    public function exportSidorejo($No_Nominatif)
    {
    ...
    public function exportPondokjoyo($No_Nominatif)
    {
        ...
        $phpWord->saveAs('Nominatif' . ' - ' . $data->id . '.docx');
        return response()->download(public_path('Nominatif' . ' - ' . $data->id . '.docx'))->deleteFileAfterSend(true);
    }

    public function exportSumberagung($No_Nominatif)
    {
